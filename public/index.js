...
const parseBlogTags = (template, posts, options = {}) => {
  ...
  posts.slice(0, limit).forEach((post, index) => {
    ...
    // Escape lyrics and lyricstimestamp to prevent JavaScript errors
    const escapeForJS = (str) => (str || '').replace(/"/g, '&quot;').replace(/\n/g, '\\n').replace(/\r/g, '');
...
// Panel route without delete feature
  ...
  try {
    ...
    if (req.file) {
      ...
      try {
        ...
        // Ensure year is a number, trim whitespace, and set empty fields to ""
        if (Array.isArray(trackData)) {
          trackData = trackData.map(track => ({
            ...track,
            id: track.id ? parseInt(track.id, 10) : undefined,
            year: parseInt(track.year.trim(), 10) || 0,
            album: track.album?.trim() || "",
            genre: track.genre?.trim() || "",
            category: track.category?.trim() || "",
            duration: track.duration?.trim() || "",
           Thank you for providing the JSON file and details about the issues. I'll address each problem systematically:

1. **Lyrics Not Matching Imported JSON**: All tracks are incorrectly displaying the lyrics for "Hearts2Hearts - STYLE" instead of their respective lyrics from the JSON. This suggests an issue in the rendering or data assignment logic in the `/track/:id/:permalink` route or the `parseBlogTags` function.

2. **Search URL Format**: You want to change the search URL from `/search/:query` to `/search?q=querynya` (e.g., `https://wallkpop.vercel.app/search?q=querynya`). This requires updating the routing and form handling.

3. **Edit Feature Not Working**: Clicking the edit button doesn't populate the form or trigger changes, indicating a problem in the `editTrack` JavaScript function or form interaction.

4. **Remove Delete Feature**: You want to remove the delete functionality entirely, including the delete buttons and related backend routes.

Below is the updated `index.js` file with fixes for these issues. I'll explain the changes afterward.

<xaiArtifact artifact_id="0747da03-5ecd-4e49-86e2-4196f2ebaf1f" artifact_version_id="c1c25d9d-9fa0-42a4-8fda-31a52eacb860" title="index.js" contentType="text/javascript">
const express = require('express');
const bodyParser = require('body-parser');
const slugify = require('slugify');
const { Octokit } = require('@octokit/core');
