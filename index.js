...
async function updateGitHubFile(path, content, message, sha = null, retries = 2) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const cacheKey = `github:${path}`;
      await kv.set(cacheKey, content, { ex: 86400 });
...
async function getAllTrackFiles(page = null, perPage = null) {
  try {
    ...
    if (!page) {
      await kv.set(cacheKey, fullData, { ex: 86400 });
    }
...
async function getLatestId(force = false) {
  try {
    ...
    await kv.set(cacheKey, latestId, { ex: 86400 });
    return latestId;
  } catch (error) {
...
app.get(['/', '/page/:page'], async (req, res) => {
  try {
    ...
    await kv.set(cacheKey, html, { ex: 86400 });
    res.send(html);
  } catch (error) {
